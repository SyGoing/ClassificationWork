...
def mixup_data(device,x, y, alpha=1., use_cuda=True):
...
def cutmix_data(device,x, y, alpha=1., use_cuda=True):
    if alpha > 0.:
        lam = np.random.beta(alpha, alpha)
    else:
        lam = 1.
    ...
    cut_mixed_x=x
    y_a, y_b = y, y[index]
    return cut_mixed_x, y_a, y_b, lam
...
def checkpoint(epoch,net):
    ...
    if not os.path.isdir('checkpoint'):
        os.mkdir('checkpoint')
    torch.save(net.state_dict(), './checkpoint/model_last%d.pth'%epoch)
