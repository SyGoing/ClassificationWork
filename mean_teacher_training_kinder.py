...
def train(epoch,net,model_teacher,trainloader,criterion,optimizer,device,batches_per_epoch,batchSize):
    ...
    if epoch%10==0:
        ...
        torch.save(model_teacher.state_dict(), args.model_save + '/model_teacher%d.pth' % epoch)
...
def main_train(args):
    ...
    model_student=get_resnet(class_num=args.classNums,num_layers=18)  #resnet18
    model_teacher=get_resnet(class_num=args.classNums,num_layers=18)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        test_class_loss, test_st_acc = test(model_student, testloader, class_criterion, device)
        e=time.time()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--base_lr', default=2e-5, type=float, help='base learning rate')
    ...
    parser.add_argument('--train_label', default='./DataSet/trainval_kin.txt', type=str, help='images root dir')
    parser.add_argument('--test_label', default='./DataSet/test_kin.txt', type=str, help='images root dir')
    ...
    parser.add_argument('--model_path', default="", type=str, help='model dir')
    parser.add_argument('--save_hist', default="./DataSet", type=str, help='model dir')
    parser.add_argument('--model_save',default='./checkpoint/mean_teacher')
